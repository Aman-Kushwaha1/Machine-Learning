...
class DogDataset:
    ...
    def get_transforms(self):
        if self.if_resize:
            ...
            transform_list = [ transforms.Resize((32,32)), transforms.ToTensor(),                               #0-255 to 0-1, numpy to tensors
            ...
            # Note: Only change from part a) is there is no need to resize the image 
                transform_list = [  transforms.ToTensor(), transforms.Normalize(self.x_mean, self.x_std)
                # convert image to PyTorch tensor
                # normalize the image (use self.x_mean and self.x_std)
            ]
...
class DogCatDataset:
    ...
    def get_transforms(self):
        ...
        transform_list = [ transforms.Resize((256, 256)), transforms.CenterCrop(224), transforms.ToTensor(),   #0-255 to 0-1, numpy to tensors
